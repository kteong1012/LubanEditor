...
{
    public partial class TbPath : IConfigEditorTable
    {
        private List<editor.cfg.test.Path> _datas = new List<editor.cfg.test.Path>();
        ...
        private string _name => "TbPath";
        private GUIStyle _areaStyle = new GUIStyle(GUI.skin.button);


        public TbPath(string dataFilePath)
        {
        ...
        public void Load()
        {
            ...
            if (File.Exists(_dataFilePath))
            {
                ...
                if (json.IsArray)
                {
                    ...
                    {
                        var data = editor.cfg.test.Path.LoadJsonPath(node.Value);
                        _datas.Add(data);
                ...
                {
                    var data = editor.cfg.test.Path.LoadJsonPath(json);
                    _datas.Add(data);
        ...
        private string GetTableJson()
        {
            ...
            foreach (var data in _datas)
            {
                ...
                editor.cfg.test.Path.SaveJsonPath(data, json);
                jsonArray.Add(json);
            }
            return jsonArray.ToString(4);
        }

        private string GetDataJson(editor.cfg.test.Path data)
        {
            var json = new JSONObject();
            editor.cfg.test.Path.SaveJsonPath(data, json);
        ...
        public void OnGUI()
        {
            ...
            {
                _datas.Add(new editor.cfg.test.Path());
            }
            if (GUILayout.Button("-", GUILayout.Width(20)))
            {
...
renderData.res = UnityEditor.EditorGUILayout.TextField(renderData.res, GUILayout.Width(150));
if (GUILayout.Button("…", GUILayout.Width(20)))
{
    TextInputWindow.GetTextAsync(renderData.res,__x =>renderData.res = __x);
}
UnityEditor.EditorGUILayout.EndHorizontal();    UnityEditor.EditorGUILayout.EndVertical();
}                //editor.cfg.test.Path.RenderPath(__SelectData);
            }
            GUILayout.EndScrollView();
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();
        ...
        private string GetId(editor.cfg.test.Path data)
        {
            if (data == null)
            {
        ...
        private editor.cfg.test.Path __SelectData
        {
            get
            {
                if (_selectIndex >= 0 && _selectIndex < _datas.Count)
