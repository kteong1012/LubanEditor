...
{
    public partial class TbTestMap : IConfigEditorTable
    {
        private List<test.TestMap> _datas = new List<test.TestMap>();
        ...
        private string GetDataJson(test.TestMap data)
        {
            var json = new JSONObject();
            data?.SaveJson(json);
        ...
        private string GetId(test.TestMap data)
        {
            if (data == null)
            {
        ...
        private test.TestMap SelectData
        {
            get
            {
                if (_selectIndex >= 0 && _selectIndex < _datas.Count)
